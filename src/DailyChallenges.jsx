  ...
  return (
    <div className="flex-1 flex flex-col max-w-md mx-auto w-full bg-black relative overflow-hidden">
      <div className="flex-1 overflow-y-auto flex flex-col pb-32">
        <div
          className="relative min-h-[42%] flex flex-col items-center border-b border-white/5 overflow-hidden shrink-0"
        ...
        >
          <header className="w-full px-6 pt-12 flex justify-between items-center z-10">
            <button onClick={() => setCurrentViewWithTransition('home')} className="w-10 h-10 flex items-center justify-center bg-zinc-900/50 rounded-full text-yellow-500">
              <Icons.Chevron dir="left" />
            </button>
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-zinc-300">Daily Challenges</h2>
            <div className="w-10" />
          </header>
          ...
          <div className="flex-1 flex flex-col items-center justify-center z-10 pt-4 pb-6 w-full">
            <div className="relative flex items-center justify-center w-full">
               <button
                 ...
                 className="absolute left-4 z-20 pointer-events-auto text-yellow-500/30 hover:text-yellow-500 active:text-yellow-500 transition-colors disabled:opacity-0"
               >
               ...
               <button
                 disabled={cMonth===11}
                 onClick={()=>setCMonth(m=>m+1)}
                 className="absolute right-4 z-20 pointer-events-auto text-yellow-500/30 hover:text-yellow-500 active:text-yellow-500 transition-colors disabled:opacity-0"
               >
            ...
            <div className="mt-6 text-center">
               <h3 className="text-3xl font-black italic uppercase tracking-[0.2em] text-white">
                 {MONTHS[cMonth]} 2026
               </h3>
        ...
        <div className="flex-1 px-8 pt-8 no-scrollbar pb-10 relative">
          <div className="flex justify-between items-end mb-8">
            <span className="text-2xl font-black text-white italic tracking-[0.1em]">{MONTHS[cMonth]} 2026</span>
            <div className="flex items-center gap-2 bg-zinc-900 px-3 py-1.5 rounded-full border border-yellow-500/10">
              <Icons.Trophy size={14} fill="#EAB308" />
              <span className="text-sm font-bold tabular-nums text-zinc-200">0/31</span>
            </div>
          </div>
          <div className="grid grid-cols-7 text-center mb-4 opacity-30 text-[11px] font-bold uppercase">
          ...
          <div className="absolute bottom-0 left-8 right-8 z-20 pb-4">
            <button onClick={()=>start('Daily',true)} className="w-full bg-yellow-500 text-black py-5 rounded-full font-black text-xl shadow-2xl active:scale-95 transition">Play</button>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-zinc-950/80 backdrop-blur-xl border-t border-white/5 flex justify-around p-4 pb-8 z-50 pointer-events-auto">
        <button onClick={()=>setCurrentViewWithTransition('home')} className="flex flex-col items-center gap-1.5 text-zinc-600">
          <Icons.Nav type="main" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Main</span>
        </button>
        <button className="flex flex-col items-center gap-1.5 text-yellow-500">
