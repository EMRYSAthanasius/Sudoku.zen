...
export function DailyChallenges({ cMonth, setCMonth, MONTHS, calendarDays, startDaily, setCurrentViewWithTransition }) {
  return (
      ...
      <div className="flex-1 overflow-y-auto flex flex-col pb-32">
        ...
        <div className="flex-1 px-8 pt-8 no-scrollbar pb-10 relative">
          ...
          <div className="absolute bottom-0 left-8 right-8 z-20 pb-4">
            <button onClick={()=>startDaily()} className="w-full bg-[#C19A6B] text-[#2D1B10] py-5 rounded-[24px] font-black text-xl shadow-2xl border-b-4 border-[#A0522D] active:border-b-0 active:translate-y-1 transition-all duration-150">Play</button>
          </div>
        </div>
      </div>
